#!/usr/bin/env python3
"""SpecAutoencoder Training: Train a deep convolutional autoencoder on CodecFake audio data."""
...
# SAFE SpecAugment (waveform-preserving)

def apply_spec_augment(waveform):
    """
    IMPORTANT:
    This version does NOT convert to spectrogram to avoid
    breaking the autoencoder architecture.

    It applies light waveform masking instead.
    """
    if waveform.ndim != 2:
        return waveform
    ...
    # Time masking (simulate dropout in time domain)
    for _ in range(2):
        width = random.randint(0, int(0.1 * length))
        start = random.randint(0, max(0, length - width))
        waveform[:, start:start + width] = 0
...
class CodecFakeMultiClassDataset(Dataset):
    def __init__(self, root_dir, seed=None, target_len=80000, augment=False):
        self.samples = []
...
class DeepAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(1, 32, 9, 2, 4), nn.BatchNorm1d(32), nn.ReLU(),
            nn.Conv1d(32, 64, 9, 2, 4), nn.BatchNorm1d(64), nn.ReLU(),
            nn.Conv1d(64, 128, 9, 2, 4), nn.BatchNorm1d(128), nn.ReLU(),
            nn.Conv1d(128, 256, 9, 2, 4), nn.BatchNorm1d(256), nn.ReLU(),
        )

        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(256, 128, 9, 2, 4, 1), nn.BatchNorm1d(128), nn.ReLU(),
            nn.ConvTranspose1d(128, 64, 9, 2, 4, 1), nn.BatchNorm1d(64), nn.ReLU(),
            nn.ConvTranspose1d(64, 32, 9, 2, 4, 1), nn.BatchNorm1d(32), nn.ReLU(),
            nn.Dropout(0.3),
            nn.ConvTranspose1d(32, 1, 9, 2, 4, 1), nn.Tanh()
        )
...
def main():
    ...
    parser.add_argument(
        "--data-dir",
        default="/ocean/projects/cis260121p/shared/codecfake_pt"
    )

    parser.add_argument(
        "--save-path",
        default="/ocean/projects/cis260121p/shared/models/spec_autoencoder.pt"
    )

    parser.add_argument(
        "--checkpoint-dir",
        default="/ocean/projects/cis260121p/shared/checkpoints/spec_autoencoder"
    )
    ...
    parser.add_argument(
        "--use-augment",
        action="store_true",
        help="Enable waveform-based SpecAugment"
    )
