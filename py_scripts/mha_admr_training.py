#!/usr/bin/env python3
"""MHA-ADMR Training: Audio Deepfake Model Recognition — classify by generation model (6 classes)."""
...
class MHA_ADMR_model(nn.Module):
    """MHA-ADMR model: pretrained encoder + attention + 6-class classifier."""
    def __init__(self, pretrained_autoencoder):
        super(MHA_ADMR_model, self).__init__()
...
def train_MHA_ADMR_model(model, train_loader, val_loader, device, epochs=10, lr=1e-4,
                     save_path="models/MHA_ADMR_model.pt", checkpoint_dir="checkpoints/MHA_ADMR",
...
def main():
    ...
    parser.add_argument("--save-path", type=str, default="/ocean/projects/cis260121p/shared/models/MHA_ADMR_model.pt")
    parser.add_argument("--csv-dir", type=str, default="/ocean/projects/cis260121p/shared/csv/MHA_ADMR_split")
    parser.add_argument("--checkpoint-dir", type=str, default="/ocean/projects/cis260121p/shared/checkpoints/MHA_ADMR")
    ...
    model = MHA_ADMR_model(pretrained_autoencoder=autoencoder).to(device)
    ...
    # Step 4: Train
    train_MHA_ADMR_model(model, train_loader, val_loader, device,
    ...
    evaluate_model(model, test_loader, device,
                   report_path=os.path.join(args.working_dir, "reports/MHA_ADMR/test_report.csv"),
                   plot_path=os.path.join(args.working_dir, "plots/MHA_ADMR/"))

    # Step 6: Confidence analysis
    conf_csv = os.path.join(args.working_dir, "csv/MHA_ADMR/confidence_scores.csv")
